...
## Always opens the insertion UI — the player must place the item and Continue.
...
func apply(caster: EnemyInstance, target: Node, params: Array) -> void:
	...
	## Skip if this parasite is already lodged (sticky grenades may stack).
	if (
		item_id != "STICKY_GRENADE"
